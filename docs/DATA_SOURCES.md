...
| Volume (rows) | ~20M+ mutations (France) |
...
| Raw file size | ~4-5 GB (full France SQL dump) |
| License | Licence Ouverte v2.0 (free reuse) |
| Dictionary | https://doc-datafoncier.cerema.fr/doc/dv3f/ |
| Projection (geometry) | EPSG varies by region (2154 metro, 2975 Reunion, etc.) |
| Encoding | UTF-8 |
...
|---------|----------------------------|--------------------|
| Access | Free, open | Convention required (SAFER, collectivites) |
...
| Attribute | Value |
|-----------|-------|
| Source name | Contours administratifs (communes + departements) |
...
| Recommended file | `communes-1000m.geojson` (~10 MB, 1km generalization -- fast + good enough for dashboard) |
...
| **Estimated volume** | ~20M rows (France, 2014-2025) |
...
| `idnatmut` | integer | NO | FK to ann_nature_mutation (sale type) | 1 |
...
| `sterr` | decimal | YES | Total land area sold (m2) | 850.00 |
...
| `sbati` | decimal | YES | Total built area sold (m2) | 120.00 |
| `sbatmai` | decimal | YES | Total house area sold (m2) | 120.00 |
| `sbatapt` | decimal | YES | Total apartment area sold (m2) | 0.00 |
...
- `geomlocmut` may be NULL for rural areas with poor geocoding
...
| **Primary key** | `iddispo` |
| **Foreign key** | `idmutation` to `mutation` |
| **Estimated volume** | ~25M rows |
...
| `idmutation` | integer | NO | FK to mutation |
...
| **Foreign keys** | `iddispo` to `disposition`, `idparcelle` to `parcelle`, `idmutation` to `mutation` |
| **Estimated volume** | ~25M rows |
...
| `iddispo` | integer | NO | FK to disposition |
| `idparcelle` | integer | NO | FK to parcelle |
| `idmutation` | integer | NO | FK to mutation |
...
| **Foreign keys** | `iddispopar` to `disposition_parcelle`, `idmutation` to `mutation` |
| **Estimated volume** | ~15M rows |
...
| `iddispopar` | integer | NO | FK to disposition_parcelle |
| `idmutation` | integer | NO | FK to mutation |
...
| `sbati` | decimal | YES | Built surface (m2) |
| `coddep` | varchar | NO | Department code |
| `datemut` | date | NO | Transaction date |
| `anneemut` | integer | NO | Transaction year |
...
| **Primary key** | `idsuf` |
| **Foreign key** | `iddispopar` to `disposition_parcelle` |
| **Estimated volume** | ~30M rows |
...
| `dsuf` | decimal | YES | Surface of the subdivision (m2) |
...
| Table | Grain | PK | FK | Est. Rows |
|-------|-------|----|----|-----------|
| `lot` | One row per lot in a disposition | `idlot` | `iddispo` to disposition | -- |
| `volume` | One row per volume (vertical co-ownership) | `idvol` | `iddispo` to disposition | -- |
| `mutation_article_cgi` | Link mutation to tax article | `idmutart` | `idmutation` to mutation | -- |
| `adresse_dispoparc` | Link address to disposition_parcelle | composite | `iddispopar`, `idadr` | -- |
| `adresse_local` | Link address to local | composite | `iddispoloc`, `idadr` | -- |

---

### 3.2 ANNEXE TABLES (reference data)

| Table | PK | Content | Example Values |
|-------|----|---------|---------------|
| `ann_nature_mutation` | `idnatmut` | Mutation type labels | 1="Vente", 2="Vente en l'etat futur d'achevement", 3="Echange", 4="Vente de terrain a batir", 5="Adjudication" |
| `ann_type_local` | `codtyploc` | Premises type labels | 1="Maison", 2="Appartement", 3="Dependance", 4="Local industriel..." |
| `ann_cgi` | `artcgi` | Tax code article references | "257-7-1-2", "683", etc. |
| `ann_nature_culture` | `natcult` | Land use type labels | "T"="Terre", "P"="Pre", "VE"="Verger", "VI"="Vigne", "BT"="Bois taillis" |
...
─────────────────────────
departements.geojson ─────→ stg_geo__departments ────────────────────────────────────────────→ dim_geography
communes.geojson ─────────→ stg_geo__communes ───────────────────────────────────────────────→ dim_geography

ann_nature_mutation ───────→ BigQuery raw table (reference)
ann_type_local ────────────→ BigQuery raw table (reference)
ann_nature_culture ────────→ BigQuery raw table (reference)
```
...
| `commune_name` | STRING | geo_communes GeoJSON `properties.nom` | From GeoJSON join on commune code |
...
| Rule | Table | Column(s) | Type | Threshold |
|------|-------|-----------|------|-----------|
...
| FK integrity | disposition | idmutation | relationship | 0 orphans |
| FK integrity | disposition_parcelle | iddispo | relationship | 0 orphans |
| FK integrity | local | iddispopar | relationship | 0 orphans |
| Department code | mutation | coddep | accepted_values | Valid FR dept codes |
| Property type | mutation | codtypbien | not_null | For analysis subset |
| Geometry validity | mutation | geomlocmut | custom | Check null rate |
| GeoJSON dept coverage | geojson_departements | code | completeness | All 101 FR departments present |
| GeoJSON commune coverage | geojson_communes | code | completeness | All communes from DVF+ have a match |
| Join integrity | dim_geography | geo_code to fct_transactions.department_code | relationship | 0 unmatched departments |

---

## 8. BigQuery-Specific Considerations

### PostGIS to BigQuery Migration

| PostGIS Column | BigQuery Handling |
|---------------|-------------------|
| `geomlocmut` (point) | Extract as `latitude` (FLOAT) + `longitude` (FLOAT) via `ST_Y()`, `ST_X()` |
| `geomparmut` (polygon) | Drop for this project (parcel boundaries not needed for dashboard) |
| `geompar` (polygon) | Drop for this project |

### PostgreSQL Array Columns to BigQuery

| PG Column | Type | BigQuery Handling |
|-----------|------|-------------------|
| `l_codinsee` | varchar[] | Unnest to first element or REPEATED STRING |
| `l_dcnt` | decimal[] | Drop (detailed surfaces available via suf table) |
| `l_idpar` | varchar[] | Drop (available via disposition_parcelle join) |
| `l_artcgi` | varchar[] | Drop (available via mutation_article_cgi join) |
